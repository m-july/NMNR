...
class My_Parameters:
    ...
    def get(self, index):
        """获取参数值"""
        if self.randomize and self.params.loc[index, 'make_noise']:
...
example_monophonic_seq_0_short = {
    'notes': [
        {
            'note_type': 'number',
            'number': 1,
    ...
    def compile_line_views_to_ele(self, views, line_split_info, PARAMS):
        ...
        for view, line_ready, underlines_ele, curves_ele in zip(views, lines_ready, underlines_eles, curves_eles):
            ...
            if ele.physical_box.is_valid():
                ...
                off_y += PARAM.get('LAYOUT_LINE_EMPTY_ROW_HEIGHT') / 2
            ele.position += Position(0, off_y)
            result.add(ele)
            if ele.physical_box.is_valid():
                off_y += ele.physical_box.down + PARAMS.get('LAYOUT_LINE_SPACE')
            else:
                off_y += PARAM.get('LAYOUT_LINE_EMPTY_ROW_HEIGHT') / 2 + PARAMS.get('LAYOUT_LINE_SPACE')
...
PARAMS_.randomize = False

temp = EMonophonic(example_monophonic_seq_0_short, PARAMS_, appog=False, line_split_info={
    'width_target': 2000,
    'w_to_k_ratio': 2,
    'x_k_threshold': 20,
})
...
# %%
temp.render_to_path("tmp.png", debug=False, padding=100)
